...
/*
************************************************************
* PROCESO: PL_CONSULTA_PARSER_PM1_ITEM_VERSION
*
* DESCRIPCION: Este proceso se genera para realizar una
*              CONSULTA!!! sobre los precios, de la tabla
*               PM1_ITEM_VERSION 
************************************************************
*/
...
Declare
    ...
    RowInsPcio TFA_PRECIOS_ITEM_VERSION_UAT%ROWTYPE :=NULL; --Se necesita una tabla TMP_PRECIOS_ITEM_VERSION
    checkingTablaTmp  NUMBER :=0;
    vRelease_Seq_No NUMBER(10) :=&1;
    vOffer_Eff_Date VARCHAR2(8):='&2';
    ...
    and a.PACKAGE_ID in (select offer  from Bl1_version_details where  cycle_code=1 and release_seq_no=vRelease_Seq_No and OFFER_EFFECTIVE_DATE=to_date(vOffer_Eff_Date,'YYYYMMDD'))
...
--        dbms_output.put_line(xml_string);
    ...
    --   v_Xml_Data Varchar2(4000);
       ...
       v_Speed Varchar2(50);
        ...
        v_ValidoDesde DATE;
        v_ValidoHasta DATE :=NULL;
        v_Product_Type Varchar2(6);
...
--       v_largo:=length(xml_string)-62; -- le saco la primer parte y la ultima parte, dejando solo el xml puro.
        v_largo:=INSTR(xml_string,'</ItemParameters>') +  length('</ItemParameters>') - 45;
--       dbms_output.put_line(v_largo);
...
--              Dbms_Output.Put_Line('v_Value=' || v_Value);
              -- hasta acá ya tengo el name, elementaryType y el value, para uno solo.
                If v_Name = 'Item ID' then
                    ...
                    BEGIN
                        select distinct product_type into v_Product_Type 
                        from csm_offer 
                        where soc_cd in (select soc_cd from csm_offer_item where item_cd=v_ID);
                    EXCEPTION 
                        when others then
                       DBMS_OUTPUT.PUT_LINE(SUBSTR('ERROR: '||v_ID||' ',1,200));
                     END; 
...
--                        Dbms_Output.Put_Line('v_DimensionName=' || v_DimensionName);
                        if (v_DimensionName = 'Speed' OR v_DimensionName = 'Package ID' OR v_DimensionName = 'Number of Units' )  then
                            v_Speed:=v_SubValue;
                        elsif (v_DimensionName = 'Central group ID' OR v_DimensionName = 'Device gama type' OR v_DimensionName = 'Type' ) then
                    ...
                    v_NewLineTotal:=v_ID||'|'||v_Charge_code||'|'||v_Speed||'|'||v_CentralGroup||'|'||v_Precio||'|'||v_Type||'|'|| v_ValidoDesde||'|'||v_Product_Type ;
                   ...
                   BEGIN
                    ...
                    RowInsPcio.VALID_TO                :=v_ValidoHasta;
                    RowInsPcio.PRODUCT_TYPE        :=v_Product_Type;

                    INSERT INTO TFA_PRECIOS_ITEM_VERSION_UAT
                    VALUES RowInsPcio;
                    COMMIT;
            ...
            End Loop;
        End Loop;
        v_NewLineTotal:=v_ID||'|'||v_Charge_code||'|'||v_Speed||'|'||v_CentralGroup||'|'||v_Precio||'|'||v_Type||'|'|| v_ValidoDesde;
        Dbms_Output.Put_Line(v_NewLineTotal);
                    BEGIN
                    RowInsPcio.ID                            := v_ID;
                    RowInsPcio.CHARGE_CODE          := v_Charge_code;
...
--       DBMS_OUTPUT.PUT_LINE('Pase x printout ok');
