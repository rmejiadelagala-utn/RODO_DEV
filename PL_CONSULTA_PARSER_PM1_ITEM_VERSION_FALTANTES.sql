...
Declare
    ...
    ) and id  in (select PRICING_ITEM_ID 
from prdcustc.APE3_EPCEXT_PRICING_ITEMS where --PRICING_ITEM_ID= 154838077 and
trunc(sys_creation_date)='26-Oct-2019' and ITEM_PARAM_ATTR_NAME='Charge code' and PACKAGE_EXP_DATE is null
and   PRIT_TYPE in ('**TEF Flat or Percentage RC Discount',
    '**TEF Flat or Percentage RC Discount for component',
    '**TEF RC Insurance flat rate', -- lo de insurance lo manejamos aparte por la Gama.
    '**TEF RC based on Package ID',
    '**TEF RC based on Package ID for WRLS',
    ...
    '**TEF RC flat rate for multiple components')
minus
select ID from tfa_precios_item_version)
...
--		 Busco si la oferta existe en  
          BEGIN
                Select MAX(PACKAGE_EFF_DATE) 
                INTO v_ValidoDesde
                from prdcustc.APE3_EPCEXT_PRICING_ITEMS where PACKAGE_ID=vPACKAGE_ID and PACKAGE_EXP_DATE is null;
