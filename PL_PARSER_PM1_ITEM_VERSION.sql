...
--                        Dbms_Output.Put_Line('v_DimensionName=' || v_DimensionName);
                        if v_DimensionName = 'Speed' then
                            v_Speed:=v_SubValue;
                        elsif v_DimensionName = 'Central group ID' then
                    ...
                    End Loop;
                    v_NewLineTotal:=v_ID||'|'||v_Charge_code||'|'||v_Speed||'|'||v_CentralGroup||'|'||v_Precio||'|'||v_Type;
